...
var PlayPart = React.createClass({
  ...
  render: function () {
    ...
    return React.createElement(
      'div',
      { className: 'playPart part', onClick: this.handleClick },
...
function findElement(array, name, value) {
  for (var i = 0; i < array.length; i++) if (array[i][name] == value) return array[i];
}
...
var ChannelListPart = React.createClass({
  ...
  render: function () {
    ...
    var channelNodes = this.props.channels.map(function (channel) {
      return React.createElement(ChannelPart, { url: channel.url, number: channel.id, key: channel.id, itemSelected: that.handleItemClick, name: channel.name, img: channel.img });
    });

    return React.createElement(
      'div',
      { className: 'channelPart part' },
...
var ChannelPart = React.createClass({
  ...
  render: function () {
    return React.createElement(
      'li',
      { url: this.props.url, key: this.props.key, number: this.props.number, onClick: this.onClick },
...
var SettingsPart = React.createClass({
  ...
  render: function () {
    ...
    return React.createElement(
      'div',
      { className: 'settingsPart part', onClick: this.handleClick },
      React.createElement('span', { className: resumeClass })
    );
  }
});

React.render(React.createElement(RadioBox, null), document.getElementById('content'));
